  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 className="text-3xl font-semibold text-[#2f2418]">Content Queue</h1>
          <p className="mt-1 text-sm text-[#8a7a67]">Draft, schedule, and generate ideas for the selected profile.</p>
        </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {ideas.map((idea) => (
              <div key={idea.title} className="rounded-2xl border border-[#e2d6c2] bg-[#fffaf2] p-4">
                ...
                <p className="mt-3 text-sm text-[#8a7a67]">{formatHashtags(idea.hashtags)}</p>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-[#e2d6c2]">
                  {filteredItems.map((item) => (
                    <tr key={item.id}>
                      <td className="py-4 pr-4 text-[#2f2418]/90">{item.caption.slice(0, 84)}{item.caption.length > 84 ? "..." : ""}</td>
                      ...
                      <td className="py-4 pr-4 text-[#8a7a67]">{item.scheduledAt ? format(new Date(item.scheduledAt), "MMM d, yyyy h:mm a") : "Not set"}</td>
                      <td className="py-4">
                        <Button
                          variant="ghost"
                          size="sm"
              ...
              {filteredItems.length === 0 ? <div className="py-8 text-center text-[#8a7a67]">No content found for this status.</div> : null}
            </div>
          )}
        </CardContent>
      </Card>
